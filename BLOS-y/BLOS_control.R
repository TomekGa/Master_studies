...
#need to have dominance and selection coefficient from data you want to compare with
...
for(rep in random.replicates){
  ...
  wybrany.samiec <- samplex(males,size = 1)
  wybrana.samica <- samplex(females,size = 1)
  samica <- populacja[wybrana.samica,1:G]
  samiec <- populacja[wybrany.samiec,1:G]
  populacja <- populacja[-c(wybrana.samica,wybrany.samiec),]
  ...
  if(length(samiec)>0){
    ...
    file.name4 <- paste("BLOS",rep,"_control.txt",sep = "")
...
for(rep in 1:l.replikatow){
  ...
  file.name <- paste("BLOS",rep,"_control.txt",sep="")
  ...
  if(nrow(samice)>=2){
    for(bred in breeding){
      ...
      if(length(samiec)>0){
        ...
        populacja1 <- cbind(populacja1,c(samplex(c(0,1),size=nrow(populacja1),replace = T)))
        file.name5 <- paste("BLOS",bred,rep,"_control.txt",sep = "")
